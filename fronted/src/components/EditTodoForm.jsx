...
const EditTodoForm = () => {
  ...
  useEffect(() => {
    if (!searchParams.get("id")) {
      ...
      axios
        .get("http://localhost:3000/todos/" + searchParams.get("id"))
        .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (todoTitle && todoDescription) {
      axios
        .put("http://localhost:3000/todos/" + searchParams.get("id"), {
          title: todoTitle,
  ...
  return (
    <div className="bg-gray-100 w-[90%] md:w-[80%] lg:w-[50%] p-5 rounded-sm shadow-md">
      ...
      <form
        ...
        <input type="text" className="w-full border-2 border-gray-500 px-5 py-2 rounded-sm my-2" placeholder="Enter Todo" value={todoTitle} onChange={(e) => setTodoTitle(e.target.value)} />

        {/* TODOS DESCRIPTION INPUT FIELD */}
        <textarea className="w-full border-2 border-gray-500 px-5 py-2 rounded-sm" cols="30" rows="3" placeholder="Enter Description" value={todoDescription} onChange={(e) => setTodoDescription(e.target.value)} />
